...
namespace WindowsFormsApp1
{
    public partial class FormKitaplar : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                filtreler.Add(" tur = @turum ");
                sql.komut.Parameters.AddWithValue("turum", comboBox2.SelectedItem.ToString());
            }
        ...
        private void yazdir()
        {
            ...
            while (oku.Read())
            {
                ...
                {
                    string[] yukle = { oku.GetString(0), oku.GetString(1), oku["tur"].ToString(), "Ödünç Alınabilir" };
                ...
                else if (oku["durum"].ToString() == sql.adim)
                {
                    ...
                    string[] yukle = { oku.GetString(0), oku.GetString(1), oku["tur"].ToString(), "Senin Elinde" };
                ...
                else
                {
                    ...
                    if (sql.adim == "admin")
                    {
                         yukle = new string[] { oku.GetString(0), oku.GetString(1), oku["tur"].ToString(), oku["durum"].ToString() };
                    }
                    else
                    {
                        yukle = new string[] { oku.GetString(0), oku.GetString(1), oku["tur"].ToString(), "alınamaz" };
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if(textBox1.Text != "" && textBox2.Text != "" && comboBox2.SelectedIndex != 0)
            {
                sql.Baglantı();
                sql.komut.Parameters.Clear();
                sql.komut.CommandText = "select * from Table_kitaplar where ad = @adim and yazar = @yazarım and tur = @turum";
                sql.komut.Parameters.AddWithValue("adim",textBox1.Text);
                sql.komut.Parameters.AddWithValue("yazarım",textBox2.Text);
                sql.komut.Parameters.AddWithValue("turum",comboBox2.SelectedItem);
                SqlDataReader oku = sql.komut.ExecuteReader();
                if (oku.Read())
                {
                    sql.baglan.Close();
        ...
        private void buttonsil_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "" && textBox2.Text != "" && comboBox2.SelectedIndex != 0)
            {
                ...
                else
                {
                    sql.baglan.Close();
                    sql.Baglantı();
                    sql.komut.CommandText = "insert into Table_kitaplar (ad,yazar,tur) values (@adim,@yazarim,@turum)";
                    sql.komut.Parameters.AddWithValue("adim", textBox1.Text);
